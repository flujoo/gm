---
title: "Complete Guide to mr"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Complete Guide to mr}
...
knitr::opts_chunk$set(
  ...
  # add `mr_vignette=true` into it
  # to build the vignette locally
  eval = identical(Sys.getenv("mr_vignette"), "true")
)
...
With mr, you can create musical scores and audio files easily, and show them
in R Markdown files, R Jupyter Notebooks and RStudio.
...
let's first learn how to install and configure mr.
...
into it. You can open the file with `usethis::edit_r_environ()`.

Now below is a complete guide to mr.


## `Music` Objects

In mr, we use `Music` objects to represent music.
...
mr interprets MIDI note numbers based on keys and neighbor pitches. In the
...
duration types defined in mr are:
...
You can also append what we call "tupler" notations in mr to a duration
...
you get `1/3`, but this in mr is not a valid *duration value*. We put this
...
`"1"`, which is invalid in mr. This is also to avoid ambiguity. For example,
...
Yes, there is something wrong with this score. MuseScore (which mr uses
internally) has some bugs, so it can't convert MusicXML (which mr uses to
...
`Clef` objects you can create in mr are:
...
mr automatically adds a clef to a staff based on its pitch contents, if
the staff has no clef at the beginning. For example, in the following score,
mr adds an alto clef to the staff:
...
mr provides a simple and intuitive language, with which you can express
complex music easily. One possible usage of mr is algorithmic composition,
