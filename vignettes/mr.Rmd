...
We can insert it into the third measure at the second beat:
...
Below is a `Music` object that consists of only one `Line` object:
...
We can add a `Line` object to a `Music` object in many different ways.

For example, we can add `Line` "b" as another **part**:
...
We can make `Line` "b" come before `Line` "a", by specifying argument
`to` and `after`:
