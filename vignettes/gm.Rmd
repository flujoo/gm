...
for (i in seq_along(keys)) music <- music + Key(keys[i], bar = i)
show(music)
```
...
`Key(1, to = 1)` adds a G major to the first musical line and it affects the whole part, while `Key(-1, to = 2, scope = "staff")` adds an F major to the third musical line and it affects only that corresponding staff.
